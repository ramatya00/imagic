...
import { GUEST_GENERATION_LIMIT, GUEST_COOKIE_NAME, COOKIE_MAX_AGE } from "../lib/constants";
	...
	if (authenticated) {
		try {
			...
			return {
				...
				message: user.credits === 0 ? "No credits remaining. Please purchase more." : undefined
			};
		} catch (error) {
			return {
				allowed: false,
		...
		return {
			...
			isGuest: true,
			remaining: remaining,
			message: remaining === 0 ? "Free generation used. Sign up for more!" : undefined
		};
	}
}